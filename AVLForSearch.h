...
{
	int bf;
	vector<file> data;		//存放文件位置
	K key;			
	AVLNode<T, K>* left, * right;
	AVLNode() :left(NULL), right(NULL),bf(0){ }
	AVLNode(vector<file> d, K k, AVLNode<T, K>* l = NULL, AVLNode<T, K>* r = NULL)
	{
		right = r;
		left = l;
		key = k;
	...
	AVLNode(T d, K k,int lin, AVLNode<T, K>* l = NULL, AVLNode<T, K>* r = NULL)
	{
		data.push_back(file(d,lin));
		key = k;
		right = r;
		left = l;
		bf = 0;
...
public:
	...
	bool insert(const K& key, const T& val,int lin) { return insert(key, val,lin , root); }
	bool remove(const K& key, const T& val) { return remove(key, val, root); }
	bool search(const K& key,vector<file>& pos) { return search(key,root, pos); }
	
	
	void preOrder() { preOrder(root); }
protected:
	
	bool insert(const K& key, const T& val,int lin , AVLNode<T, K>*& root);
	bool remove(const K& key, const T& val, AVLNode<T, K>*& root);
	bool search(const K& key, AVLNode<T, K>* root,vector<file>& pos);
...
template<class T, class K>
inline bool AVLTree<T, K>::insert(const K& key, const T& val,int lin, AVLNode<T, K>*& root)
{
	...
	while (ptr != nullptr)
	{
		if (ptr->key == key) 
		{
			...
			for (int i = 0; i < ptr->data.size(); i++)
				if (ptr->data[i].data == val)
					notcontain = false;
			if (notcontain)
				ptr->data.push_back(file(val,lin));
			return false;
		}
		pre = ptr;
		track.push(pre);
	...
	ptr = new AVLNode<T, K>(val, key,lin);
	assert(ptr != nullptr);
	if (pre == nullptr)
	{
		root = ptr;
...
template<class T, class K>
inline bool AVLTree<T, K>::search(const K& key, AVLNode<T, K>* root,vector<file>& pos)
{
	...
	while (ptr != nullptr)
	{
		if (ptr->key == key)
		{
			for (int i = 0; i < ptr->data.size(); i++)
			{ 
				pos.push_back(file(ptr->data[i].data,ptr->data[i].line));
